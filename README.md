# Hubert Walczak

I build TypeScript libraries, Counter-Strike tools, and the occasional project that gets a little out of hand.

A lot of my work starts with game data: parsing demos, making sense of live match state, and turning it into something useful for developers and broadcasts. I also enjoy working on binary formats, streaming APIs, and making things run in the browser.

## Libraries

| Project | What it does |
| --- | --- |
| [cs2parser](https://github.com/osztenkurden/cs2parser) | Parses Counter-Strike 2 demos and live GOTV broadcasts, with typed access to players, entities, and game events. Runs in Node.js, Bun, and browsers. |
| [bzip2-codec](https://github.com/osztenkurden/bzip2-codec) | Dependency-free bzip2 compression and decompression for JavaScript, with Web Streams and optional WebAssembly decoding. |
| [csgogsi](https://github.com/osztenkurden/csgogsi) | Turns CS2 and CS:GO Game State Integration data into structured match state and typed gameplay events. |
| [steam-game-path](https://github.com/osztenkurden/steam-game-path) | Finds installed Steam games by App ID across Steam libraries on Windows, macOS, and Linux. |

## Broadcast Tools

I've contributed to [CS2 React HUD](https://github.com/lexogrine/cs2-react-hud), an open-source spectator overlay for [LHM.gg](https://github.com/lexogrine/hud-manager). It's where game data meets the screen: radar, player information, and custom broadcast interfaces.

## Side Quests

Not everything needs to be a library. My [Heroes III map editor](https://hwalczak.com/homm3-editor/) started as a wallpaper generator and grew into a tool for reading, editing, and exporting game maps. There's also a [Heroes III soundtrack player](https://hwalczak.com/homm3/) for a little background nostalgia.

More projects and live experiments at [hwalczak.com](https://hwalczak.com).
